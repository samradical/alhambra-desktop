...
const ASSETS_DIR = "https://storage.googleapis.com/samrad-alhambra/desktop/"
...
function TourOverlay(state, prev, send) {
  ...
  let imgsrc = `${ASSETS_DIR}Location_${padVal}/location_${padVal}_${Locations[state.activeLocation].name}.jpg`
  ...
  if (prevLocation !== state.activeLocation) {
    ...
    ANIM_TO = setInterval(function() {
      ...
      if (timeCounter < 0) {
        ...
        document.getElementById('alhambra-app').style.display = "block";
      }else{
  ...
  return html `
    <div class="tour--overlay" style="width: ${state.map.alhambraAppWidth}px; height: ${state.map.alhambraAppHeight}px; left: ${left}px; top:${top}px;">
